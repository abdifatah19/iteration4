...
{
$rid=intval($_GET['statussuccess']);
$sql=mysqli_query($con,"UPDATE tblsellbooks SET status='1' WHERE id='$rid'");
echo "<script>alert('Request Accepted');</script>";
echo "<script type='text/javascript'> document.location ='managesellingrequest.php'; </script>";
}
if(isset($_GET['statusreject']))
{
$rid=intval($_GET['statusreject']);
$sql=mysqli_query($con,"UPDATE tblsellbooks SET status='2' WHERE id='$rid'");
                      ...
                      <div class="content-panel table-responsive">
                          <table class="table ">
                              ...
                              <td><?php echo $cnt;?></td>
                                  ...
                                  <td style="width:10%;">
                                 <?php  if($row['status'] == "0") { ?>
                                    <a href="?statussuccess=<?php echo ($row['id']);?>" class="btn btn-success btn-xs " title="approve" data-toggle="tooltip" onclick="return confirm('Do you really want to Approve request ?');"><i class="fa fa-check "></i></a>  
                                  <a href="?statusreject=<?php echo ($row['id']);?>" class="btn btn-danger btn-xs " title="reject" data-toggle="tooltip" onclick="return confirm('Do you really want to Reject request ?');"><i class="fa fa-close "></i></a> 
